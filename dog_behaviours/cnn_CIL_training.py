...
from utils import load_config, load_processed_dog_data, evaluate_all_classes, set_seed
...
from models.cnn_head import ClassRegistry
import logging
...
def main():
    ...
    test_loader = DataLoader(TensorDataset(X_test, y_test_remapped), batch_size=32, shuffle=False)
    ...
    for task_idx, task_classes in enumerate(cil_tasks):
        ...
        train_loader = DataLoader(TensorDataset(X_task, y_task_remapped), 
                                batch_size=cfg['CIL_batch_size'], shuffle=True)
    ...
    make_CIL_plots(backbone_class_acc, accuracy_history)
