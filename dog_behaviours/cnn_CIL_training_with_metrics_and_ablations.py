...
import logging
from cnn_CIL_training_utils import load_CIL_data, CIL_post_task_eval, train_with_simplified_tdm, train_with_simplified_tdm_no_replay, make_CIL_plots, check_CIL_validity
...
def main():
    ...
    if cfg['replay']:
        ...
        cil_tasks =  [["Sniffing"], ["Trotting"], ["Galloping"]]
    ...
    print("DEBUG: Complete evaluation mapping (behavior  local ID):")
    for i, behavior in enumerate(all_behaviors):
        gid = behavior_to_idx[behavior]
        local_id = m_test[gid].item()
        print(f"  {behavior} (GID {gid})  local ID {local_id}")
    print()
    ...
    backbone_all7_acc, class_acc, sample_counts, correct_counts = evaluate_all_classes(backbone_model, test_loader, backbone_registry, all_behaviors, device)
    ...
    #Calculate backbone accuracy on seen classes only (sample-weighted average)
    backbone_total_samples = sum([sample_counts[cls] for cls in backbone_behaviors])
    backbone_correct_samples = sum([correct_counts[cls] for cls in backbone_behaviors])
    backbone_seen_acc = 100.0 * backbone_correct_samples / max(backbone_total_samples, 1) if backbone_total_samples > 0 else 0.0
    
    print(f"Backbone accuracy on seen classes only: {backbone_seen_acc:.2f}%")
    print(f"Backbone seen classes sample counts: {[(cls, sample_counts[cls]) for cls in backbone_behaviors]}")
    ...
    #Add backbone training data to replay buffer
    print("DEBUG: Adding backbone training data to replay buffer...")
    with torch.no_grad():
        backbone_model.eval()
        for batch_x, batch_y in backbone_train_loader:
            batch_x = batch_x.to(device)
            batch_y = batch_y.to(device)
            
            #Get features from backbone
            feats = backbone_model._features(batch_x)  #(batch_size, feature_dim)
            
            #Add to replay buffer
            replay_buffer_q.add_batch(feats, batch_y)
    
    print(f"DEBUG: Replay buffer populated with {len(replay_buffer_q)} samples")
    print(f"DEBUG: Replay buffer counts per class: {replay_buffer_q.counts_per_class()}")
    ...
    all_seen_classes = set(backbone_behaviors)
    ...
    for task_idx, task_classes in enumerate(cil_tasks):
        ...
        #For new-class-only learning, task_classes contains only the new class
        new_cls = task_classes  #task_classes now contains only the new class
        cil_model.expand_head(new_cls, registry)
        
        #Debug: Print head size after expansion
        print(f"DEBUG: Head size after expansion: {cil_model.head.out_dim}")
        print(f"DEBUG: Registry num classes: {registry.num_classes()}")
        print(f"DEBUG: Registry seen classes: {registry.seen_classes()}")

        #Update all_seen_classes to include current task classes
        all_seen_classes.update(task_classes)

        cil_model.eval()

        #2) Inter-task expansion (warm up grow before epochs)
        if task_idx > 0 and cfg['adaptive_head']:
        ...
        if cfg['replay']:
            #For replay, only train on the NEW class, old classes come from replay buffer
            new_class_only = [new_class]  #Only the new class for this task
            map_task, task_gids = make_global_to_local_map(new_class_only, behavior_to_idx, device=y_train.device)
        else:
            #No-replay ablation - use evaluation mapping for consistency
            map_task = m_test  #Use the same mapping as evaluation
            task_gids = torch.tensor([behavior_to_idx[cls] for cls in task_classes], device=y_train.device)

        #Debug: Print the mapping for training classes
        if cfg['replay']:
            training_classes = new_class_only
        else:
            training_classes = task_classes
            
        print(f"DEBUG: Training mapping for task classes:")
        for i, cls in enumerate(training_classes):
            gid = behavior_to_idx[cls]
            local_id = map_task[gid].item()
            print(f"  {cls} (GID {gid})  local ID {local_id}")

        mask = torch.isin(y_train, task_gids)
        X_task = X_train[mask]
        y_task = y_train[mask]
        y_task_remapped = map_task[y_task]   #vectorized mapping
        
        #Debug: Print unique classes in training data
        unique_classes = torch.unique(y_task_remapped).tolist()
        print(f"DEBUG: Unique classes in training data: {unique_classes}")
        
        #Map local IDs back to behavior names
        if cfg['replay']:
            #For replay, use new_class_only indexing
            training_behaviors = [new_class_only[i] for i in unique_classes if i < len(new_class_only)]
        else:
            #For no-replay, map local IDs back to behavior names using evaluation mapping
            training_behaviors = [all_behaviors[i] for i in unique_classes if i < len(all_behaviors)]
        
        print(f"DEBUG: Training on behaviors: {training_behaviors}")
        print(f"DEBUG: Training label mapping: {[(i, all_behaviors[i], behavior_to_idx[all_behaviors[i]]) for i in unique_classes if i < len(all_behaviors)]}")
        ...
        print(f"DEBUG: Training data shape: X_task={X_task.shape}, y_task_remapped={y_task_remapped.shape}")
        print(f"DEBUG: Training labels range: {y_task_remapped.min().item()} to {y_task_remapped.max().item()}")
        print(f"DEBUG: Unique training labels: {torch.unique(y_task_remapped).tolist()}")
        
        #Debug: Check what the model will return
        sample_batch = next(iter(train_loader))
        sample_x, sample_y = sample_batch
        print(f"DEBUG: Sample batch: x={sample_x.shape}, y={sample_y.shape}")
        print(f"DEBUG: Sample labels: {sample_y.tolist()[:5]}")  #First 5 labels
        ...
        for epoch in range(cfg['CIL_epochs']):
            ...
            if epoch > 0 and epoch % delta_k == 0 and cfg['adaptive_head']:
                cil_model.head.update_mask_intra(p = p_intra)
   
            #Inter-task warm-up end (shrink back to target sparsity)
            if task_idx > 0 and epoch == delta_k and cfg['adaptive_head']:
                cil_model.head.shrink_after_warmup(p=p_inter)
                
            #Train and get metrics
            if cfg['replay']:
                #For replay, pass only the new class to the training function
                train_loss, train_acc = train_with_simplified_tdm(cil_model, cfg, registry, new_class_only, teacher, prev_num, train_loader, behavior_to_idx, optimizer, criterion, device, 
                                        replay_buffer_q, seen_gids.tolist(), seen_names, task_idx, epoch)
            else:
                #For no-replay, use a modified training function that handles evaluation mapping
                train_loss, train_acc = train_with_simplified_tdm_no_replay(cil_model, cfg, registry, task_classes, teacher, prev_num, train_loader, behavior_to_idx, optimizer, criterion, device, seen_gids.tolist(), seen_names, task_idx, epoch)
        ...
        #Debug: Print which classes are being evaluated
        print(f"DEBUG: Evaluating on {len(all_behaviors)} total classes:")
        for i, behavior in enumerate(all_behaviors):
            gid = behavior_to_idx[behavior]
            local_id = m_test[gid].item()
            is_seen = behavior in all_seen_classes
            in_registry = behavior in registry.seen_classes()
            print(f"  Class {local_id}: {behavior} (GID {gid}) - {'SEEN' if is_seen else 'UNSEEN'} - {'IN_MODEL' if in_registry else 'NOT_IN_MODEL'}")
        
        print(f"DEBUG: Model registry contains {len(registry.seen_classes())} classes: {list(registry.seen_classes())}")
        print("DEBUG: Model registry mapping (behavior  registry row):")
        for behavior in registry.seen_classes():
            row = registry.row_for_global[behavior]
            print(f"  {behavior}  registry row {row}")
        ...
        for class_id in sorted(eval_classes):
            behavior_name = all_behaviors[class_id]
            print(f"  Class {class_id}: {behavior_name} - {eval_samples_per_class[class_id]} samples")
        
        #Debug: Check model's ability to predict the new class specifically
        if not cfg['replay']:  #Only for no-replay ablation
            print(f"DEBUG: Checking model's ability to predict new class: {new_class}")
            new_class_gid = behavior_to_idx[new_class]
            new_class_local_id = m_test[new_class_gid].item()
            print(f"DEBUG: New class GID: {new_class_gid}, Local ID: {new_class_local_id}")
            print(f"DEBUG: Is new class in registry? {new_class in registry.seen_classes()}")
            if new_class in registry.seen_classes():
                print(f"DEBUG: New class registry row: {registry.row_for_global[new_class]}")
        
        #Debug: Test a few predictions manually to see what's happening
        print("DEBUG: Testing manual predictions...")
        cil_model.eval()
        with torch.no_grad():
            #Get a small batch
            sample_batch = next(iter(test_loader))
            sample_x, sample_y = sample_batch
            sample_x = sample_x[:5].to(device)  #First 5 samples
            sample_y = sample_y[:5].to(device)
            
            #Get predictions using forward_eval7
            logits = cil_model.forward_eval7(sample_x, registry)
            predictions = logits.argmax(dim=1)
            
            print(f"DEBUG: Sample ground truth labels: {sample_y.tolist()}")
            print(f"DEBUG: Sample predictions: {predictions.tolist()}")
            print(f"DEBUG: Logits shape: {logits.shape}")
            print(f"DEBUG: Logits for first sample: {logits[0].tolist()}")
            print(f"DEBUG: Max logit value: {logits.max().item():.3f}")
            print(f"DEBUG: Min logit value: {logits.min().item():.3f}")
            
            #Check if any backbone classes are being predicted
            backbone_classes = [0, 1, 2, 3]  #Standing, Walking, Sitting, Lying chest
            backbone_predictions = [p.item() for p in predictions if p.item() in backbone_classes]
            print(f"DEBUG: Backbone class predictions: {backbone_predictions}")
        ...
        seen_classes_acc = {cls: all_classes_acc[cls] for cls in all_seen_classes}
        
        #Calculate proper weighted overall accuracy for seen classes only
        seen_total_samples = sum([sample_counts[cls] for cls in all_seen_classes if cls in sample_counts])
        seen_correct_samples = sum([correct_counts[cls] for cls in all_seen_classes if cls in correct_counts])
