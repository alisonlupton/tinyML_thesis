...
def calculate_cl_metrics_from_matrix(R, random_baseline=None, matrix_type="cumulative"):
    ...
    if random_baseline is None:
        ...
        # This is a rough approximation - you might want to compute actual random baselines
    ...
    metrics['intransigence'] = Rstar[-1] - R[-1, -1]   # paper’s I_K
    ...
    # Modified BWT (κ): κ_K = (2/(K(K-1))) * sum_{i=2}^K sum_{j=1}^{i-1} (R_{i,j} - R_{j,j})
    if K > 1:
        kappa = 0
    ...
    # Modified FWT (ζ): ζ_K = (2/(K(K-1))) * sum_{i<j} R_{i,j}
    if K > 1:
        zeta = 0
        for i in range(K):
