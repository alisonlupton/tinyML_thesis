...
class BalancedQuantReplayDynamic:
    def __init__(self, cfg, buffer_size: int, feat_dim: int):
        ...
        self.hard_frac = cfg['hard_frac']      #percent of class buffer allocated to 'hard' samples
        self.eps = cfg['eps']         #replace the weakest hard example if the new one is >= eps better
        self.rho = cfg['rho']    #chance to admit to anchors even if reservoir replacement wouldnt (extra rho percent sample stays), stops resevoir from going stale
        self.hard_ratio = cfg['hard_ratio']
    ...
    def class_summary(self, cid: int):
        ...
        #stats for display: combine hard+anchor scores if want
    ...
    def sample_q(self, batch_size: int, seen_class_ids, device="cpu"):
        ...
        hard_ratio = self.hard_ratio #percentage of replay sampled from hard selection
        ...
        for cid in seen:
            ...
            #sample from HARD uniformly (or weighted if want)
            if take_h > 0:
