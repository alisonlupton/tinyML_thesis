...
def main():
    ...
    #Extract and save features for CL metrics calculation
    print("Extracting features for CL metrics calculation...")
    cil_model.eval()
    ...
    for task_idx, _task_gids in enumerate(schedule):        
        ...
        #Update all_seen_gids to include current task classes
        current_task_gids = [int(task_local_to_gid[cls].item()) for cls in range(len(task_local_to_gid))]
        all_seen_gids.update(current_task_gids)
